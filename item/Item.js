...
import { ランダムな1要素, 整数乱数, 空文字列, 範囲 } from "../Util.js";
...
export class アイテム extends 取引アイテムインターフェース {
  ...
  static 初期化() {
    ...
    this.#一覧 = new Map([
      ...
      new 非戦闘時の道具("ｽｷﾙの種", 1000, (プレイヤー) => {
        const 増加値 = 整数乱数(3, 1, true);
        プレイヤー.現職.SP増減(増加値);
        プレイヤー.チャット書き込み予約(`${プレイヤー.名前}のSPが ${増加値} あがった！`);
        return true;
      }),
      new 非戦闘時の道具("幸せの種", 3000, (プレイヤー) => {
        // TODO
        プレイヤー.経験値 = プレイヤー.レベル * プレイヤー.レベル * 10;
        プレイヤー.チャット書き込み予約(`次のクエスト時にレベルアップ！`);
        return true;
      }),
      new 非戦闘時の道具("小さなﾒﾀﾞﾙ", 500, (プレイヤー) => {
        プレイヤー._小さなメダル.収支(1);
        プレイヤー.チャット書き込み予約("メダル王にメダルを１枚献上しました");
        return true;
      }),
      ...
      new 非戦闘時の道具("ﾌｧｲﾄ一発", 3000, (プレイヤー) => {
        プレイヤー._疲労 = 0;
        プレイヤー.チャット書き込み予約(`元気全快！${プレイヤー.名前}の疲労が回復した！`);
      }),
...
class 使用可能な道具インターフェース extends 道具 {
   ...
   * @param {string} 名前
   * @param {number} [価値]
   * @param {(import("../character/Character.js").メンバー) => boolean} [効果]
   */
...
class ステータスの種 extends 非戦闘時の道具 {
  ...
  _効果(プレイヤー) {
    ...
    プレイヤー.ステータス.増減(増加値);
    プレイヤー.チャット書き込み予約(`${プレイヤー.名前}の${this.#成長ステータス名}が ${増加値} あがった！`);
    return true;
  }
...
class 変装用の道具 extends 非戦闘時の道具 {
  ...
  _効果(プレイヤー) {
    プレイヤー._アイコン = this.#アイコン名;
    プレイヤー.チャット書き込み予約(`${プレイヤー.名前}は${this.#姿の名前}のコスプレをした！`);
    return true;
  }
...
class オーブ extends 非戦闘時の道具 {
  ...
  _効果(プレイヤー) {
    if (プレイヤー.オーブを捧げる(this)) {
      プレイヤー.チャット書き込み予約(`${this.名前}を復活の祭壇にささげた！`);
      return true;
    }
    else {
      通知欄.追加("すでにささげられています");
...
class 福袋 extends 非戦闘時の道具 {
  ...
  _効果(プレイヤー) {
    ...
    プレイヤー.現在地.NPCに話させる(`なんと、${this.名前}の中身は ${中身名} だった！`);
    プレイヤー.アイテム倉庫.追加(中身名);
    return true;
  }
...
class 進む人のステータス依存 {
  ...
  ランダム取得(進んだプレイヤー) {
    const ステータス = 進んだプレイヤー.ステータス[this.#ステータス名];
    return 整数乱数((
